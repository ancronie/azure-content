...
Then you need to add a declaration for Background Tasks in your `Package.appxmanifest` file :

- go on `Declarations` 
- go on `Available Declarations` 
- select `Background Tasks` 
- click the `Add` button 
- go under `Properties` 
- check `Push notification` 
- in `Entry point` add "EngagementBackgroundTasks.RawNotificationBackgroundTask". 
